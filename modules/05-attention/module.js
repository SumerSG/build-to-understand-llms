...
\`gradCheckAttention(layer, x, opts = {})\`: call \`gradCheck\` from \`lib/tensor.js\` with inputs \`[x, ...layer.parameters()]\` (the caller passes \`x\` with \`requiresGrad: true\`, for example made with \`Tensor.param\`, so hand it straight to \`gradCheck\` without copying or wrapping it) and the scalar loss \`layer.forward(x).pow(2).sum()\`, and return its result. Because the parameters are the layer's own Tensors, perturbing them in place changes what \`forward\` computes, so one call checks the input gradient and every parameter gradient at once. A check that only passes \`x\` would miss a broken \`Linear\` backward; the test counts the inputs covered through each \`details\` entry's \`input\` field (its position in the inputs array), so it expects 5 distinct values.
`,
      hints: [

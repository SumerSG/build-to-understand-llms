...
export default async function demo(m, lab) {
  ...
  lab.done(`Your loop trained a ${run.model.numParams().toLocaleString()}-parameter GPT for **${config.steps} steps** on **${run.tokensSeen.toLocaleString()} tokens** in ${seconds.toFixed(1)} s (**${Math.round(tokensPerSec).toLocaleString()} tokens/s**). Train loss fell from ${first.toFixed(3)} (uniform guessing is ln ${tokenizer.vocabSize} = ${uniform.toFixed(3)}) to **${last.toFixed(3)}** nats/char; final validation loss **${finalVal.toFixed(3)}**, ${verdict}. Clipping fired on ${clipped} of ${config.steps} steps. Final sample: "${samples.at(-1).text.slice(0, 80)}…"`);
}

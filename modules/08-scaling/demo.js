...
export default async function demo(m, lab) {
  ...
  const extraPct = (100 * llama.extraTrainingFlops / llama.optimal.flops);
...
At a fixed budget the loss is U-shaped in N: for C = 1e23 FLOPs the fit puts the minimum at **${(optima[1].N / 1e9).toFixed(2)}B parameters** and **${(optima[1].D / 1e12).toFixed(2)}T tokens**, about **${optima[1].tokensPerParam.toFixed(0)} tokens per parameter**. Llama-3 8B used **${plans[2].tokensPerParam.toFixed(0)}** — over-trained by **${extraPct.toFixed(0)}%** more training compute than the ${(llama.optimal.N / 1e9).toFixed(0)}B-parameter model that reaches the same predicted loss. That buys **${llama.savingPerInferenceToken.toExponential(2)} FLOPs saved per served token**, so the trade pays for itself after **${llama.breakEvenInferenceTokens.toExponential(2)} inference tokens** — a number a production endpoint passes in weeks.`);
}

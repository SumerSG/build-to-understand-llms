export default {
  ...
  recall: [
    ...
    { q: 'In module 07, what does AdamW\'s division by `sqrt(v)` achieve?', options: ['It normalises the loss', 'Every parameter moves by roughly `lr` per step, whatever the scale of its gradient', 'It prevents overflow'], answer: 1,
      why: 'Your GRPO loop reuses AdamW. Because the step size is roughly `lr` regardless of gradient scale, even a tiny spurious gradient (say 1e-6 from a NaN-guard or a rounding error, still well above AdamW\'s `eps` of 1e-8) would move a weight by about `lr` on a fresh optimizer. Only an exactly zero gradient adds nothing to the first and second moments: on a fresh optimizer that is a zero step, and mid-run the weights keep drifting only on the momentum earlier batches built up, with nothing new from this group. That is why an all-equal group must produce advantages of exactly 0 (one of the tests).' },
  ],
  review: [
...
`,
      ...
      hints: [
        'A regular expression finds the number: an optional minus sign, digits, an optional fraction. `Number(match)` converts it. For verify, compare the two numbers numerically, not as strings (the answers here are integers, so `===` on the parsed values is exact), and remember that `parseAnswer` may return `null`.',
...
\`clippedLoss(logp, oldLogp, adv, clip = 0.2)\`: the PPO objective. Form the ratio \`r = exp(logp − oldLogp)\` as a Tensor (\`oldLogp\` is a plain array of constants). For each sample decide, with plain numbers, which branch of \`min(r·A, clamp(r, 1−clip, 1+clip)·A)\` is smaller. Where the unclipped branch wins, the contribution is \`r_i · A_i\` and the gradient flows through \`r\`; where the clipped branch wins, the contribution is the constant \`clamp(r_i)·A_i\` and there is no gradient. Inside \`[1−clip, 1+clip]\` the two branches are equal; count that as unclipped, so the gradient flows (at \`r = 1\` the loss must match \`reinforceLoss\` in value and gradient). Return \`−mean(...)\` as a scalar Tensor. The work of this step is getting the gradient to flow through some samples and not others inside one Tensor expression; the hints show one way if you get stuck.
`,
      hints: [
        ...
        '`const ratio = /* exp(logp − oldLogp), with oldLogp wrapped as a constant Tensor */; for (i) { const r = ratio.data[i], A = adv[i]; const clamped = Math.min(Math.max(r, 1 - clip), 1 + clip); if (/* clipped branch is the smaller one */) frozen[i] = clamped * A; else live[i] = A; } return ratio.mul(new Tensor({ shape: [N], data: live })).add(new Tensor({ shape: [N], data: frozen })).mean().neg();`',
      ],
    },
    {
      id: 'kl',
...
`,
      predict: { question: 'The reference equals the policy and the rollout\'s rewards are all 0. After one grpoStep, how much did the weights move?', answer: 'Not at all. Every advantage is 0, so the clipped loss has zero gradient; and at policy = reference the k3 gradient `1 − exp(0)` is 0 too. A fresh AdamW turns a zero gradient into a zero step. (Mid-run, the momentum left by earlier iterations would still move the weights a little, but this batch adds nothing to it.) GRPO learns nothing from prompts it always fails, or always solves.' },
      hints: [

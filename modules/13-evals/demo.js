...
Finally, **${contam.flagged.length} of ${tasks.length} prompts** shared a 13-gram with the corpus; dropping them changes pass@1 from ${p1.value.toFixed(3)} to **${cleanPass1.toFixed(3)}**. Widening the eval from 5 to 40 tasks shrank the interval from ${widths[0].toFixed(3)} to ${widths[3].toFixed(3)}, close to the ${Math.sqrt(8).toFixed(1)}x that 1/sqrt(tasks) predicts.`);
}

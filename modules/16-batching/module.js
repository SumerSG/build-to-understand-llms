export default {
  ...
  review: [
    ...
    { q: 'Two requests send the identical 40-token prompt and blockSize is 16. How many blocks can they share?', options: ['3', '2', '0'], answer: 1, why: 'Only the two full blocks. The third block holds 8 prompt tokens and will be written with each request\'s own generated tokens, so it is copied — copy-on-write on the last partial block, exactly as vLLM does it.' },
  ],
  concept: `
...
\`tFixed\` does not care how big the batch is: the weights must be read out of HBM whatever happens. For an 8B model in bf16 that is approximately 16 GB at the approximately 3.35 TB/s of HBM3 bandwidth in NVIDIA's H100 datasheet, about 5 ms. \`tPerToken\` is the arithmetic: approximately \`2N\` FLOPs per token (module 08), roughly 40 us at an assumed 350 TFLOP/s. **The fixed term is 100 times the per-token term** — the most important number in serving.
...
`,
      ...
      hints: [
        ...
        '`return s.prefilled ? 1 : s.promptLen + s.generated;` for the first, `cfg.tFixed + cfg.tPerToken * tokens` for the second, and `s.prefilled ? s.promptLen + s.generated - 1 : 0` for the third.',
      ],
    },
    {
      id: 'static',
...
`,
      hints: [
        ...
        '`const cached = cache.get(key); const shared = cached ? Math.min(cached.length, full) : 0; const fresh = total - shared; if (alloc.freeCount < fresh) return null;` then retain, allocate the tail, concatenate, and `if (!cached && full > 0) { const entry = table.slice(0, full); alloc.retain(entry); cache.set(key, entry); }`',
      ],
    },
  ],
  reflection: [
  ...
  stretch: [
    'Add chunked prefill: cap the tokens per iteration at `maxBatchedTokens` and split a long prompt across several iterations, so a 4,000-token prefill never stalls everyone\'s decode. Measure TPOT before and after. This is Sarathi-Serve (Agrawal et al. 2024), now the default scheduler in vLLM.',
    'Replace recompute preemption with swapping: pay a PCIe transfer for the victim\'s blocks (approximately 64 GB/s over PCIe 5 x16) instead of re-prefilling them, and find the sequence length where each policy wins. vLLM implements both.',

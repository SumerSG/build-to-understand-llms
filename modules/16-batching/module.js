...
`,
      predict: { question: 'Static and continuous batching run the same trace and do the same number of iterations in some cases. Which trace makes them identical, and why?', answer: 'Any trace where maxBatch is 1, or where every request arrives at once with the same outputLen. With staggered arrivals continuous batching admits latecomers mid-batch and the schedules diverge even when the lengths match. Padding only exists when members of a batch finish at different times, and with one slot there is nothing to share. The gap grows with the spread of output lengths, which is why the tests use a distribution where 80% want 4 tokens and 20% want 120.' },
      hints: [
...
\`internalFragmentation(lengths, blockSize)\` — given the sequence lengths resident in the cache, return \`{ tokens, slots, wasted, fraction }\` where \`tokens\` is \`sum(L)\`, \`slots\` is \`sum(ceil(L / blockSize) · blockSize)\`, \`wasted = slots − tokens\` and \`fraction = wasted / slots\` (0 when \`slots\` is 0). Run it with \`blockSize = 1024\` and you have modelled the contiguous-slab allocator vLLM replaced.
`,
      hints: [
...
**Growth and preemption.** Before costing the iteration, every prefilled running request needs room for one more entry: \`appendToken(s.blocks, kvTokens(s))\`. When that fails, preempt the **most recently admitted** running request (the last element of \`running\`): free its blocks, set \`prefilled = false\`, keep its \`generated\` count, increment \`restarts\`, and \`unshift\` it onto the front of \`waiting\` so it is readmitted first. If the victim is the request you were trying to grow, it simply leaves the batch; otherwise retry the append. The victim is always the last element, even when it was admitted this iteration and has not been prefilled yet: it still counts as a preemption and its \`restarts\` still goes up. This is preemption by **recomputation** — when it comes back it re-processes \`promptLen + generated\` tokens, which is what your step-1 \`tokensThisIteration\` already returns.

**Extra metrics.** Alongside the step-3 fields report \`preemptions\`, \`recomputedTokens\` (tokens processed by re-prefills of requests with \`generated > 0\`), \`peakBlocks\` (the maximum of \`usedCount\`) and \`wastedFraction\` (1 minus the ratio of total KV tokens to total allocated slots, summed over iterations). Take both samples once per iteration, after the per-request token update and before retirement: compare \`usedCount\` with \`peakBlocks\`, add \`usedCount · blockSize\` to the slot total and \`kvTokens(s)\` of every running request to the token total. \`wastedFraction\` is 0 if no slots were ever sampled.
...
4. Otherwise \`retain\` the shared blocks, allocate the fresh ones, and return \`{ blocks, sharedBlocks, newBlocks }\`, where \`blocks\` is the block table (the shared ids first, then the fresh ids) and \`sharedBlocks\` and \`newBlocks\` are the **numbers** \`shared\` and \`fresh\`, not arrays.

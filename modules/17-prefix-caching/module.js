...
3. the child's key matches for \`c\` blocks and then diverges — **split** it into a head of length \`c\` and a tail holding the rest, then continue from the head.
...
\`breakEvenHitRate(pricing)\`: solve \`h * cacheRead + (1 - h) * cacheWrite = base\` for \`h\`. Return 0 rather than a negative number when the cache is free to write.

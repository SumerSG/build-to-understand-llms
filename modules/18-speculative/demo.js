...
Against the checkpoint, the bigram draft was accepted **${(100 * K4.alpha).toFixed(1)}%** of the time at K = 4, giving **${K4.tokensPerStep.toFixed(2)} tokens per target pass** (model: ${m.expectedTokensPerStep(K4.alpha, 4).toFixed(2)}); the most tokens per target pass in the K sweep came at K = ${bestSweep.K} (${bestSweep.r.tokensPerStep.toFixed(2)}), while the speedup model at c = ${cBigram} fed the measured α, which assumes every position is accepted at the same rate, would pick K = ${rows[0][5]}. The sharpened self-draft reached α = **${(100 * selfRun.alpha).toFixed(1)}%** and ${selfRun.tokensPerStep.toFixed(2)} tokens per pass, but at c = 1 the model gives it only **${rows[1][3].toFixed(2)}×**, against **${rows[0][3].toFixed(2)}×** for the almost-free bigram draft. With the same bigram draft, α was ${(100 * tempRuns[0].alpha).toFixed(1)}% / ${(100 * tempRuns[1].alpha).toFixed(1)}% / ${(100 * tempRuns[2].alpha).toFixed(1)}% at target temperature 0.5 / 1 / 2: α = Σ min(p, q) peaks where the target's shape is closest to the draft's, and moves whichever way the temperature pulls p away from it. In a compute-bound regime (rho = 1) both drafts model at under 1× (${rows[0][4].toFixed(2)}× and ${rows[1][4].toFixed(2)}×): speculation only pays where verification is nearly free.

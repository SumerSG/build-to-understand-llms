...
`,
  ...
  stretch: [
    'Add full activation recomputation: a flag that multiplies `compute` by about 8/6 ≈ 1.33 (one extra forward pass, 2N FLOPs per token, on top of the 6N of forward plus backward) and replaces the stage\'s activation term with the layer inputs only, `2 · microTokens · dModel / tp` bytes per layer for each micro-batch kept (sequence parallelism shards the inputs too), plus one layer\'s working set (`34 · microTokens · dModel / tp` bytes) while that layer is being recomputed. Korthikanti et al. 2022 call this *full* recomputation. Their *selective* recomputation re-runs only the attention core, the `5·a·s/h` attention-matrix term, which this module\'s 34-bytes-per-token FlashAttention model already leaves out, so it would change nothing here. Rerun `bestPlan` with full recomputation and see whether tensor parallelism is still needed.',

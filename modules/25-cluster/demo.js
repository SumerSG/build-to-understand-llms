...
**Mixture of experts:** spreading ${moe.name}'s experts over 8 nodes costs ${free[epNodes.indexOf(8)].toFixed(2)} ms of all-to-all per layer; node-limited routing to ${moe.maxNodes} nodes brings that to ${limited[epNodes.indexOf(8)].toFixed(2)} ms, a ${((1 - limited[epNodes.indexOf(8)] / free[epNodes.indexOf(8)]) * 100).toFixed(0)}% cut in total all-to-all time, without changing which experts the token uses. The inter-node bytes fall further, ${((1 - wideLimited.interNodeBytes / wide.interNodeBytes) * 100).toFixed(0)}% (${mb(wide.interNodeBytes)} to ${mb(wideLimited.interNodeBytes)} per GPU per layer); the total falls less because the NVLink fan-out is unchanged.

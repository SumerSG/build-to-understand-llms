export default async function demo(m, lab) {
  ...
  const SECONDS = 120;
  const N = 2600;
  lab.md(`**How to read this:** the same two minutes of simulated chat traffic are served four ways on the same GPUs; the service promises the first word within ${SLO.ttft} s and each later word within ${(1000 * SLO.tpot).toFixed(0)} ms (the SLO, service-level objective), "attainment" is the share of requests that kept both promises, and the best arrangement is the one with high attainment at a low "$ / M tokens" (dollars per million output tokens, the last column of the big table).`);
  ...
  lab.done(`Same ${work.length} requests: three arrangements of the same ${REPLICAS} GPUs, plus one fleet the autoscaler sizes between ${m.DEFAULT_CONFIG.minReplicas} and ${m.DEFAULT_CONFIG.maxReplicas}.
...
**Cost per million output tokens** (the table's last column): ${runs.map((r) => `${r.name.split(' (')[0]} **$${m.costPerMillionTokens(r.res.gpuSeconds, r.rep.outputTokens, m.DEFAULT_CONFIG.dollarsPerGpuHour).toFixed(2)}** at ${(100 * r.rep.attainment).toFixed(0)}% attainment`).join('; ')}.`);
}

// Serving at scale — reference solution: serving at cluster scale.
//
...
export function remainingWork(s) {
  const prefill = s.prefilled ? 0 : Math.max(0, s.promptLen - s.cached);
  return prefill + Math.max(0, s.outputLen - s.generated);
}
...
export function replicaLoad(r) {
  let total = 0;
  for (const s of r.queue) total += remainingWork(s);
  for (const s of r.running) total += remainingWork(s);
  return total;
}
...
export function buildRing(ids, { vnodes = DEFAULT_CONFIG.vnodes } = {}) {
  if (!ids.length) throw new Error('buildRing: need at least one replica id');
  const ring = [];
  for (const id of ids) for (let v = 0; v < vnodes; v++) ring.push({ hash: hash32(`${id}#${v}`), id });
  ring.sort((a, b) => a.hash - b.hash || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
  return ring;
}
...
export function pickOnRing(ring, key) {
  if (!ring || !ring.length) throw new Error('pickOnRing: the ring is empty; build it from the ready replicas first');
  const h = hash32(String(key));
  let lo = 0, hi = ring.length;
  while (lo < hi) { const mid = (lo + hi) >> 1; if (ring[mid].hash < h) lo = mid + 1; else hi = mid; }
...
export function chooseReplica(policy, candidates, req, ctx = {}) {
  if (!candidates || !candidates.length) {
    throw new Error('chooseReplica: no eligible replica — the balancer must hold the request until one is ready');
  }
  const cfg = ctx.cfg || DEFAULT_CONFIG;
  if (policy === 'round-robin') return (ctx.dispatched || 0) % candidates.length;
  const loads = candidates.map(replicaLoad);
  const leastLoaded = () => {
    let best = 0;
    for (let i = 1; i < loads.length; i++) if (loads[i] < loads[best]) best = i;
    return best;
  };
  if (policy === 'least-loaded') return leastLoaded();
  if (policy === 'cache-aware') {
    const limit = cfg.overloadFactor * (sumArray(loads) / loads.length);
    let hit = -1;
    for (let i = 0; i < candidates.length; i++) {
      if (candidates[i].cache && candidates[i].cache.has(req.prefix) && (hit < 0 || loads[i] < loads[hit])) hit = i;
    }
    if (hit >= 0 && loads[hit] <= limit) return hit;
    if (ctx.ring && ctx.ring.length) {
      const home = candidates.findIndex((r) => r.id === pickOnRing(ctx.ring, req.prefix));
      if (home >= 0 && loads[home] <= limit) return home;
    }
    return leastLoaded();
  }
  throw new Error(`chooseReplica: unknown policy "${policy}"`);
}
...
export function kvTransferSeconds(tokens, cfg = DEFAULT_CONFIG) {
  if (tokens < 0) throw new Error(`kvTransferSeconds: tokens must be >= 0, got ${tokens}`);
  return cfg.kvSetupSeconds + (tokens * cfg.kvBytesPerToken) / cfg.kvBandwidth;
}
...
export function planPools(requests, nReplicas, cfg = DEFAULT_CONFIG) {
  if (nReplicas < 2) throw new Error(`planPools: disaggregation needs at least 2 replicas, got ${nReplicas}`);
  let prefillWork = 0, decodeWork = 0;
  for (const r of requests) {
    prefillWork += r.promptLen * cfg.tPerToken;
    decodeWork += Math.max(0, r.outputLen - 1) * (cfg.tPerToken + cfg.tFixed / cfg.maxBatch);
  }
  const total = prefillWork + decodeWork;
  let prefill = total > 0 ? Math.round((prefillWork / total) * nReplicas) : Math.floor(nReplicas / 2);
  prefill = Math.max(1, Math.min(nReplicas - 1, prefill));
  return { prefill, decode: nReplicas - prefill, prefillWork, decodeWork };
}
...
export function queueTarget(snap, cfg = DEFAULT_CONFIG) {
  const needed = Math.ceil(snap.pendingTokens / cfg.targetQueueTokens);
  return Math.max(cfg.minReplicas, Math.min(cfg.maxReplicas, needed));
}
...
export function autoscaleTarget(snap, cfg = DEFAULT_CONFIG) {
  if (snap.raw >= snap.total) return snap.raw;
  let stabilized = snap.raw;
  for (const v of snap.window || []) if (v > stabilized) stabilized = v;
  if (stabilized >= snap.total) return snap.total;
  return Math.max(stabilized, snap.total - cfg.scaleDownStep);
}
...
export function gpuSeconds(spans, endTime) {
  ...
  for (const sp of spans) {
    const end = sp.end === null || sp.end === undefined ? endTime : sp.end;
    if (end > sp.start) total += end - sp.start;
  }
  return total;
}
...
export function costPerMillionTokens(gpuSec, outputTokens, dollarsPerGpuHour = DEFAULT_CONFIG.dollarsPerGpuHour) {
  if (!(outputTokens > 0)) throw new Error('costPerMillionTokens: outputTokens must be > 0');
  return ((gpuSec / 3600) * dollarsPerGpuHour) / (outputTokens / 1e6);
}
...
export function sloReport(records, slo = { ttft: 1, tpot: 0.05 }) {
  ...
  const ttfts = [], tpots = [];
  let met = 0, outputTokens = 0, first = Infinity, last = -Infinity;
  for (const r of records) {
    const ttft = r.firstToken - r.arrival;
    ttfts.push(ttft);
    let tpot = 0;
    if (r.outputLen > 1) { tpot = (r.end - r.firstToken) / (r.outputLen - 1); tpots.push(tpot); }
    if (ttft <= slo.ttft && tpot <= slo.tpot) met++;
    outputTokens += r.outputLen;
    if (r.arrival < first) first = r.arrival;
    if (r.end > last) last = r.end;
  }
  const makespan = Math.max(last - first, 0);
  return {
    n,
    ttftP50: percentile(ttfts, 50),
    ttftP95: percentile(ttfts, 95),
    tpotP50: percentile(tpots, 50),
    tpotP95: percentile(tpots, 95),
    met,
    attainment: met / n,
    goodput: makespan > 0 ? met / makespan : 0,

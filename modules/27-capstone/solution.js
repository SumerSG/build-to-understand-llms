...
// ---------- worked examples: the template, the vocabulary, the tool ----------
...
export function buildPrompt(tokenizer, history, { budget, prefill = '' } = {}) {
  ...
  let dropped = 0;
  let text = renderChat(messages, prefill);
  ...
  while (ids.length > budget) {
    ...
    if (start < 0 || end >= messages.length) {
      throw new Error(`buildPrompt: ${ids.length} tokens with only the system prompt and the current turn; the budget is ${budget}`);
    }
    messages.splice(start, end - start);
    dropped += end - start;
    text = renderChat(messages, prefill);
    ids = tokenizer.encode(text);
  }
  return { ids, text, messages, dropped };
}
...
export function truncateCache(cache, n) {
  for (let layer = 0; layer < cache.k.length; layer++) {
    cache.k[layer] = ops.slice(cache.k[layer], 1, 0, n);
    cache.v[layer] = ops.slice(cache.v[layer], 1, 0, n);
  }
  cache.length = n;
  return cache;
}
...
export function syncCache(model, session, ids) {
  if (ids.length === 0) throw new Error('syncCache: need at least one token');
  if (ids.length > model.config.blockSize) throw new Error(`syncCache: ${ids.length} tokens exceed blockSize ${model.config.blockSize}`);
  let reused = sharedPrefixLength(session.ids, ids);
  if (reused === ids.length) reused -= 1; // the whole prompt is cached, but we still need its last logits
  truncateCache(session.cache, reused);
  const logits = prefill(model, session.cache, ids.slice(reused));
...
export function decodeReply(model, tokenizer, session, logits, { maxNewTokens = 16, stopIds = [], next, temperature = 1, topK = 0, topP = 1 } = {}) {
  if (typeof next !== 'function') throw new Error('decodeReply: pass a seeded rng function as `next`');
  const ids = [];
  let stopReason;
  while (true) {
    if (ids.length >= maxNewTokens) { stopReason = 'length'; break; }
    if (session.cache.length >= model.config.blockSize) { stopReason = 'context'; break; }
  ...
  return { text: tokenizer.decode(ids), ids, stopReason };
}
...
export function findToolCalls(text) {
  ...
  let at = 0;
  while (true) {
    const open = text.indexOf(TOOL_OPEN, at);
    if (open < 0) break;
    const close = text.indexOf(TOOL_CLOSE, open + TOOL_OPEN.length);
    if (close < 0) break;
    const body = text.slice(open + TOOL_OPEN.length, close).trim();
    at = close + TOOL_CLOSE.length;
...
export async function runToolCalls(text, tools) {
  ...
  for (const call of findToolCalls(text)) {
    const content = await tools.call(call.name, { input: call.input });
...
export async function chat(state, userMessage, { maxNewTokens = 12, prefill = '', maxToolRounds = 2, next, temperature = 1, topK = 0, topP = 1 } = {}) {
  ...
  for (let round = 0; round <= maxToolRounds; round++) {
    ...
    const gen = findToolCalls(forced).length
      ...
      : decodeReply(model, tokenizer, state.session, logits, { maxNewTokens, stopIds: state.stopIds, next, temperature, topK, topP });
    reply = forced + gen.text;
    state.history.push({ role: 'assistant', content: reply });
    rounds.push({ promptTokens: prompt.ids.length, cachedTokens: reused, prefilledTokens: prefilled, generatedTokens: gen.ids.length, dropped: prompt.dropped, stopReason: gen.stopReason });
    if (round === maxToolRounds) break;
    const results = await runToolCalls(reply, state.tools);
    if (results.length === 0) break;
    state.history.push(...results);
    toolCalls += results.length;
    forced = '';
  }
  const total = (key) => rounds.reduce((s, r) => s + r[key], 0);
  return {
    reply,
    stats: {
      rounds,
      ...
      prefilledTokens: total('prefilledTokens'),
      generatedTokens: total('generatedTokens'),
      dropped: Math.max(...rounds.map((r) => r.dropped)),
      toolCalls,
      stopReason: rounds[rounds.length - 1].stopReason,
      ms: now() - t0,
    },
  };
}

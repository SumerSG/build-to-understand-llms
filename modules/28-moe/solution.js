...
export class Router {
  ...
  forward(x) {
    const [N] = x.shape;
    const E = this.nExperts;
    const probs = this.gate.forward(x).softmax();
    const mask = ops.zeros([N, E]);
    const topIdx = [];
    for (let t = 0; t < N; t++) {
      const chosen = topKIndices(probs.data.subarray(t * E, (t + 1) * E), this.k);
      for (const e of chosen) mask.data[t * E + e] = 1;
      topIdx.push(chosen);
    }
    const kept = probs.mul(new Tensor(mask));
    const gates = kept.div(kept.sum(-1, true));
    return { probs, topIdx, gates };
  }
...
export function groupByExpert(topIdx, nExperts) {
  const lists = Array.from({ length: nExperts }, () => []);
  for (let t = 0; t < topIdx.length; t++) for (const e of topIdx[t]) lists[e].push(t);
  return lists;
}
...
export function dispatchCombine(x, gates, lists, experts) {
  ...
  for (let e = 0; e < lists.length; e++) {
    ...
    const y = experts[e].forward(gatherRows(x, rows));          // [n_e, C]: one batched call per expert
    const g = gatherRows(gates.slice(1, e, e + 1), rows);       // [n_e, 1]: this expert's gate per token
    const placed = scatterRows(y.mul(g), rows, N);              // [N, C]: zeros for tokens not routed here
    out = out === null ? placed : out.add(placed);
  }
  return out === null ? new Tensor(ops.zeros([N, C])) : out;
}
...
export function routingFractions(topIdx, nExperts) {
  ...
  let total = 0;
  for (const choices of topIdx) for (const e of choices) { f[e]++; total++; }
  return f.map((c) => (total ? c / total : 0));
}
...
export function loadBalanceLoss(probs, topIdx) {
  const E = probs.shape[1];
  const f = new Tensor(ops.raw([E], routingFractions(topIdx, E)));
  const P = probs.mean(0);
  return P.mul(f).sum().scale(E);
}
...
export function expertCapacity(nTokens, nExperts, k, factor) {
  return Math.ceil((factor * nTokens * k) / nExperts);
}
...
export function applyCapacity(lists, capacity) {
  let dropped = 0;
  const kept = lists.map((rows) => {
    if (rows.length > capacity) dropped += rows.length - capacity;
...
export class MoE {
  ...
  forward(x) {
    ...
    const N = x.size / C;
    const flat = x.reshape([N, C]);
    const { probs, topIdx, gates } = this.router.forward(flat);
    const lists = groupByExpert(topIdx, this.nExperts);
    const capacity = Number.isFinite(this.capacityFactor)
      ? expertCapacity(N, this.nExperts, this.k, this.capacityFactor)
      : N;
    const { kept, dropped } = applyCapacity(lists, capacity);
    this.lastAux = loadBalanceLoss(probs, topIdx);
    this.lastStats = { counts: lists.map((r) => r.length), dropped, assignments: N * this.k, capacity };
    return dispatchCombine(flat, gates, kept, this.experts).reshape(shape);
  }
...
export function countParams({ nEmbd, hidden, nExperts, k }) {
  const expert = 2 * nEmbd * hidden + hidden + nEmbd;
  const router = nEmbd * nExperts;
  const total = nExperts * expert + router;
  const active = k * expert + router;
  return { total, active, flopsPerToken: 2 * active };
}

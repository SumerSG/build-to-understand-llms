...
export class LoRALinear {
  ...
  constructor(base, { rank, alpha = rank, next }) {
    ...
    this.rank = rank;
    this.alpha = alpha;
    // TODO: step 1 — create this.A ([nIn, rank], Gaussian with std 1 / sqrt(nIn)), this.B ([rank, nOut], zeros),
    //       both as trainable leaves (Tensor.param), and this.scaling = alpha / rank.
    this.A = null;
    this.B = null;
    this.scaling = 1;
    // TODO: step 2 — freeze the base layer (its weight and, if present, its bias).
  }
  ...
  forward(x) {
    // TODO: step 1 — add the adapter path. For now this is just the base layer.
    return this.base.forward(x);
  }

  /** Only the adapter trains. */
  parameters() {
    // TODO: step 2 — only the adapter trains. Returning the base tensors is full fine-tuning of this layer.
...
export function trainableParameters(model) {
  // TODO: step 3 — right now this hands the optimizer everything, which is full fine-tuning.
...
export function countParams(model) {
  // TODO: step 3
  return { trainable: 0, total: 0, percent: 0 };
}
...
export function trainingMemory({ total, trainable }) {
  // TODO: step 5
  return { weights: 0, grads: 0, optimizer: 0, total: 0 };
}

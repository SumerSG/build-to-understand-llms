...
// concatTokens and forwardEmbeds, which you will call). Every function below it is yours, step by step.
...
export class PatchEmbed {
  /**
   * this.proj: Linear(patchDim, dim) with std 1 / sqrt(patchDim).
   * this.pos:  a trainable [nPatches, dim] Tensor, Gaussian with std 0.02.
   */
  ...
  /** proj's weight and bias, then pos. */
  parameters() {
    // TODO: step 2
    return [];
  }
...
export class Projector {
  /** this.fc1: Linear(dIn, hidden), std 1 / sqrt(dIn). this.fc2: Linear(hidden, dOut), std 1 / sqrt(hidden). */
  ...
  /** fc1's weight and bias, then fc2's. */
  parameters() {
    // TODO: step 2
    return [];
  }
...
export function captionTargets(nImage, captionIds, eos, width = captionIds.length) {
  // TODO: step 3
  return { textIds: captionIds.slice(), targets: [], mask: [] };
}
...
export function maskedCrossEntropy(logits, targets, mask) {
  // TODO: step 3
  return new Tensor(ops.zeros([]));
}

// ---------- step 4: contrastive alignment (CLIP) ----------
// ClipHead below is done for you; it uses your PatchEmbed. You write l2normalize and clipLoss.
...
export function clipLoss(img, txt, temperature = 0.07) {
  // TODO: step 4
  return { loss: new Tensor(ops.zeros([])), logits: img };
}
...
// Captioner is done for you: it wires your patchify, PatchEmbed and Projector together. You write
// captionLoss and caption, which use your step-3 functions and the worked forwardEmbeds.

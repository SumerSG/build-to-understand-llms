...
- \`effectiveContext(result, threshold = 0.8)\`: the longest length such that **it and every shorter length** have \`lengthCurve >= threshold\`; \`0\` if even the first fails. Stop at the first failure: a longer length that happens to recover is not usable context. (The goal demo passes \`threshold = 0.85\`, close to RULER\'s bar of Llama-2-7B\'s 85.6% at 4k; the default here is 0.8.)

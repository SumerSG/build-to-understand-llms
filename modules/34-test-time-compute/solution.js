...
// ---------- worked examples, constants and given helpers ----------
...
export function sampleStep(chain, prefix, next) {
  ...
  const right = applyOp(prev, chain.ops[i]);
  const slipped = next() < chain.errorRate;
  const tokens = BASE_TOKENS + randInt(next, HEDGE_MAX + 1) + (slipped ? DETOUR_TOKENS : 0);
...
export function sampleTrace(chain, next) {
  const steps = [];
  while (steps.length < chain.steps) steps.push(sampleStep(chain, steps, next));
  return finishTrace(steps);
}
...
export function checkSteps(chain, trace) {
  const out = [];
  let prev = chain.start;
  trace.steps.forEach((s, i) => {
    out.push(s.value === applyOp(prev, chain.ops[i]));
    prev = s.value;
  });
  return out;
}
...
export function majorityVote(answers) {
  return weightedVote(answers, answers.map(() => 1));
}

// ---------- step 3: best-of-N and outcome reward models ----------

/** The answer with the largest total score. Same abstention and tie rules as majorityVote. */
export function weightedVote(answers, scores) {
  ...
  for (let i = 0; i < answers.length; i++) {
    ...
    total.set(a, (total.get(a) || 0) + scores[i]);
  }
  for (const [a, t] of total) if (t > bestScore) { best = a; bestScore = t; }   // Map keeps first-occurrence order
  return best;
}
...
/** Extra tokens per step beyond BASE_TOKENS: the surface feature the outcome reward model latched onto. */
export function verbosity(trace) {
  return (trace.tokens - ANSWER_TOKENS) / trace.steps.length - BASE_TOKENS;
}

/** Outcome reward model: verify + λ·verbosity + σ·rmNoise('orm', chain, steps). */
export function ormScore(chain, trace, { lambda = 0.3, sigma = 0.1 } = {}) {
  return verify(chain, trace) + lambda * verbosity(trace) + sigma * rmNoise('orm', chain, trace.steps);
}

// ---------- step 4: process reward models and step-level beam search ----------

/** Per-step PRM scores: 1 for a correct step, 0 for a wrong one, plus σ·rmNoise('prm', chain, steps up to it). */
export function stepScores(chain, trace, { sigma = 0 } = {}) {
  const ok = checkSteps(chain, trace);
  return ok.map((good, i) => (good ? 1 : 0) + (sigma ? sigma * rmNoise('prm', chain, trace.steps.slice(0, i + 1)) : 0));
}
...
export function prmScore(chain, trace, opts = {}) {
  const scores = stepScores(chain, trace, opts);
...
export function stepBeamSearch({ beamWidth, depth, expand, prm }) {
  ...
  for (let d = 0; d < depth; d++) {
    const cands = [];
    for (const p of beam) for (const c of expand(p)) cands.push({ c, s: prm(c), i: cands.length });
    sampled += cands.length;
    cands.sort((a, b) => b.s - a.s || a.i - b.i);
    beam = cands.slice(0, beamWidth).map((x) => x.c);
  }
  return { best: beam[0], beam, sampled };
}
...
export function withBudget(trace, maxTokens) {
  ...
  for (const s of trace.steps) {
    if (used + s.tokens + ANSWER_TOKENS > maxTokens) break;
    kept.push(s);
    used += s.tokens;
  }
  if (kept.length === trace.steps.length) return { ...trace, truncated: false };
 ...
 * shared prefill of the prompt. Returns
...
export function ttcCost(strategy, config = DEFAULT_COST) {
  const { samples, traceTokens, promptTokens = 0 } = strategy;
  const cfg = { ...DEFAULT_COST, ...config };
  const opt = { bytesPerElement: cfg.bytesPerElement };
  const decodeTokens = samples * traceTokens;
  const decodeSteps = traceTokens;
  const peakKVBytes = cacheBytes(cfg.model, promptTokens, opt) + samples * cacheBytes(cfg.model, traceTokens, opt);
  const prefill = promptTokens > 0 ? cfg.tFixed + promptTokens * cfg.tPerToken : 0;
  const seconds = prefill + decodeSteps * (cfg.tFixed + samples * cfg.tPerToken);
  return { decodeTokens, decodeSteps, peakKVBytes, seconds, gpuSeconds: seconds * (cfg.gpus ?? 1) };
}
